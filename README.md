...
A macOS AI chat workspace with a persistent conversation rail, a dedicated
output canvas, and a focused writing mode. Send text or images, add reusable
context, choose a chat or writing operation, and pin rendered assistant replies
for copying, comparing, or revising.
...
- System-wide Control-A / Control-Q import: copies selected text from any app
  into the input or context editor while ContextDesk is running. macOS
  Accessibility permission is required the first time this is used.
- Collapsible conversation sidebar with saved messages, context, images, and
  outputs.
- Stacked or side-by-side layout (toggle via the floating Tweaks panel).
- Light & dark themes (toggle via Tweaks).
- Conversation sidebar "+" starts a new session (⌘N).
...
The repo includes an XCTest target, and a successful Xcode build remains the
main packaging verification step.
...
ContextDesk/
  ...
  Models/
    ...
    OpenAIService.swift     — provider-dispatched AI chat/writing clients
  ...
  Views/
    ...
    InputPane.swift         — chat pane + composer + operation controls
